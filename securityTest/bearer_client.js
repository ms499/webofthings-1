...
const DATA = 'Bearer eyJraWQiOiJ1dURLVTMxZWRvTi0wd0xMUnl1TW1vbmtBdi1OaFEwejZhWmxjdTN5NU8wIiwiYWxnIjoiUlMyNTYifQ'
...
function writePropertyCall(td,property, payload) {
  ...
  // write to a property
  axios.put(url, payload, { httpsAgent: agent, headers: { 'Authorization': DATA }}).then((response) => {
    if (response.status === 200) {
...
function readPropertyCall(td, property) {
  ...
  console.log(url)
  // write to a property
  axios.get(url, { httpsAgent: agent,  headers: { 'Authorization': DATA } }).then((response) => {
    if (response.status === 200) {
...
function invokeActionCall(td, action, payload) {
  ...
  // write to a property
  axios.post(url, payload, { httpsAgent: agent,  headers: { 'Authorization': DATA } }).then((response) => {
    if (response.status === 200) {
